...
$.ajax({
    ...
    success: function(data) {  
      if (data.length > 0) { 
        $.each(data.reverse(), function(i, val) {                
          $('#shots').prepend(
            '<a class="work_thumb shot" target="_blank" href="'+ val.html_url +'" title="' + val.title + '"><div class="work-text">' + '<h3>' + val.title + '</h3>' + '</div><img class="img-fluid" src="'+ val.images.hidpi +'"/></a>'
            )
        })
      }
      else {
